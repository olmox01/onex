#!/usr/bin/env python3
"""
ONEX File Manager App
File manager interattivo con interfaccia curses
"""

__title__ = "filemanager"
__description__ = "File manager interattivo con interfaccia curses"
__version__ = "1.0.0"
...
# Determina il percorso base del sistema ONEX
base_path = Path(__file__).resolve().parent.parent.parent

# Aggiungi la directory base al sys.path se necessario
if str(base_path) not in sys.path:
    sys.path.insert(0, str(base_path))

# Ottieni il percorso del filesystem virtuale tramite la variabile d'ambiente o predefinito
fs_root = os.environ.get('ONEX_FS_ROOT')
if fs_root:
    fs_root = Path(fs_root)
else:
    fs_root = base_path / "userland_fs"

# Definizione del tipo di file
class FileType(Enum):
...
class FileManager:
    ...
    def start(self) -> None:
        ...
        # Imposta la modalità raw del terminale
        os.system('stty raw -echo')
        
        try:
            # Usa curses.wrapper per gestire correttamente l'inizializzazione e la pulizia
            curses.wrapper(self._main_loop)
        except Exception as e:
            print(f"Errore durante l'esecuzione del file manager: {e}")
        finally:
            # Ripristina il terminale (anche in caso di errore)
            os.system('stty sane')
            
            # Pulisci lo schermo e posiziona il cursore
            sys.stdout.write("\033[H\033[J")  # Pulisce lo schermo
            sys.stdout.write("\033[0;0H")     # Posiziona il cursore in alto a sinistra
            sys.stdout.flush()
...
def start_file_manager(fs_root_path: Path = None, start_dir: str = "/") -> None:
    """
    Avvia il file manager.
    
    Args:
        fs_root_path: Directory radice del filesystem virtuale
        start_dir: Directory iniziale (percorso virtuale)
    """
    # Usa fs_root dalla variabile globale se non specificato
    if fs_root_path is None:
        fs_root_path = fs_root
        
    # Crea e avvia il file manager
    fm = FileManager(fs_root_path, start_dir)
    fm.start()

def main():
    """Punto di ingresso dell'applicazione."""
    print("Avvio del file manager...")
    ...
    try:
        # Avvia il file manager
        start_file_manager(fs_root, start_dir)
        return 0
    except Exception as e:
        print(f"Errore durante l'esecuzione del file manager: {e}")
        return 1
